...
**Last update:** 2026-05-18 05:23 BST — CP-9.1d `GET /v1/rca-findings` shipped:
1 fix-loop (jsonb_array_length not array_length), 5/5 green + 62/62 full
integration regression green.
**Repo:** https://github.com/vsenthil7/mendoraci (HEAD: d6f62ee + on-disk CP-9.1d)
...
| RT-007 | Analytics list views | **backend 2/5 endpoints live** 🟡 — Intakes+RCA shipped; Plans/Approvals/Evidence pending | d6f62ee + on-disk CP-9.1d | 11/11 vitest green |
| RT-008 | Secret Masking | **tested + E2E** ✅ | 35149e8 | 16 unit + cross-test |
| RT-013 | Multi-Tenant Isolation | **DB-enforced** ✅ | 226d947 | TEST-007/009/013/019/022/LST-INTAKE-4/LST-RCA-4 RLS proven on 10 tables |
| RT-015 | Idempotency & Replay | **tested + E2E** ✅ | e9320a1 | replay vitest + Pw missing-key |
| RT-009..RT-012, RT-014, RT-016..RT-020 | — | not-started | — | — |

**Roll-up: 9 / 20 RT rows fully + RT-007 partial (2/5 endpoints). Totals: 16 mask + 62 vitest integration + 6 cursor unit + 114 Playwright = 198/198 green** (Pw SCR-006 18 cases on disk; will push Pw to 132 / 216 once next full run completes).

---

## 2. CP-8 RT-006 Evidence Export (commit 1cdf8fc, pushed 03:32 BST)

ZIP bundle of masked_log.txt + rca.json + repair_plan.json + approvals.json + intake_meta.json + manifest.json, uploaded to MinIO `mendoraci-evidence` at `<tenant_id>/<intake_id>/<evidence_export_id>.zip`, returns a presigned download URL (TTL 60..3600s, default 300). Gated on `repair_plans.status='approved'`. TEST-020 proves the full pipeline incl. real MinIO upload + presigned URL download + sha256 chain + mask-at-export.

---

## 3. CP-8b — SHIPPED as commit `401ff4e` at 04:37 BST

Interim top-nav refactor + SCR-006 evidence page + 18 Playwright cases on disk. The sessionStorage-based nav is **INTERIM** and will be deleted in CP-9.5.
...
| Commit | Pushed | What |
|---|---|---|
| _on disk CP-9.1d_ | — | `GET /v1/rca-findings` route + 5 integration tests green (jsonb_array_length fix loop) |
| `d6f62ee` | 05:13 BST 18/05 | CP-9.1c `GET /v1/intakes` route + 6 integration tests + 57/57 full regression green |
...
| api | mendoraci-api (node:22-alpine + bob CLI v1.0.3) | ✅ /health 200, 8 route plugins live (intake, repo-link, rca, repair-plan, approval, evidence-export, intakes-list, rca-list) |
...
| **CP-9.1d** | `GET /v1/rca-findings` + 5 tests | 🟡 on disk, 5/5 green |
| **CP-9.1e** | `GET /v1/repair-plans` + 5 tests | 🔵 next |
...
| `GET /v1/rca-findings?…` | 🟡 on disk | paginated RCA + intake context + evidence_count + recommended_actions_count | intake_id, confidence, provider, q on root_cause, from/to |
| `GET /v1/repair-plans?…` | 🔵 next | paginated plan list | status, overall_risk, intake_id |
| `GET /v1/approvals?…` | 🔵 | paginated approval audit log | action, actor, repair_plan_id, from/to |
...
| **api vitest integration CP-9.1d rca-list** | **5** | **✅ (TEST-LST-RCA-1..5, LST-RCA-4 cross-tenant RLS proof)** |
...
| **Subtotal** | **216** | **198 green, 18 pending** |
| CP-9 remaining list-endpoint vitest (CP-9.1e/f) | +~20 | 🔵 |
